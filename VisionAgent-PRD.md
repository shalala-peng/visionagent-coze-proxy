...
> 版本：v0.4.6（原型对齐版 · 含 M3 落地设计 + 真实能力验证 + 线上 Demo 真调闭环）　|　作者：产品组　|　最后更新：2026-08-12　|　状态：原型阶段（前端 Demo，11 页可交互）＋ 真实可运行 Agent 已验证 ＋ 线上 Demo（GitHub Pages）真·Coze 实时调用打通
...
| 文档类型 | 产品需求文档 PRD |
| 当前阶段 | 方案设计 + 前端可行性验证（前端 Demo，mock 数据驱动） |
...
> 说明：原型阶段 ①②③④ 为假数据 mock（预设剧本）；真实落地需大模型 + 编排引擎（见第 10 节）。v0.4 已把"审片台 + 运行记录 + 渠道授权 + 品牌规范"做成可交互占位，闭环在 UI 层可见。

---

## 7. Skill 与路由设计

### 7.1 内置 Skill 能力池（mock 示例）
| Skill | 输入 | 输出 | 业务 |
|---|---|---|---|
| 图片分类 | 图片集 | 分类标签 | 通用 |
| 智能抠图 | 图片 + 主体参数 | 透明底图 | 电商/摄影 |
| AI 调色 | 图片 + 风格参数（可引用品牌规范） | 调色图 | 通用 |
| 场景生成 | 图 + 场景描述 | 合成图 | 广告/地产 |
| 视频粗剪 | 视频素材 | 粗剪草稿 | 旅拍/自媒体 |
| 文案生成 | 主题 + 平台（可引用品牌规范） | 标题/文案 | 自媒体/广告 |
| 合规审核 | 内容 + 规则 | 通过/不通过 + 原因 | 通用（质量门） |
| 多平台发布 | 成品 + 渠道（需渠道已授权） | 发布回执 | 通用 |
...
| 阶段 | 目标 | 状态 |
|---|---|---|
| M0 方案设计 | 信息架构、概念定义、PRD | ✅ 本文档 |
| M1 前端 Demo | 可交互原型（mock 驱动，7 页） | ✅ 已完成 |
| M1.5 原型闭环补全 | 账户与集成/审片台/运行记录/品牌中心 + 存为模板 + 角色视图 + 响应式（11 页） | ✅ v0.4 已完成 |
| M2 原型视觉稿 | 高保真设计稿（AI 原型图 / Figma / Ardot） | ⏳ 后续 |
| M2.5 真实能力验证 | 扣子可运行 Agent（NL→生产线 JSON，4 场景通过，[体验链接](https://www.coze.cn/store/agent/7672806993245192246?bot_id=true)）＋ 线上公开 Demo（[GitHub Pages](https://shalala-peng.github.io/visionagent-coze-proxy/) + 腾讯云 SCF 代理）真·Coze 实时调用，大陆浏览器可按语义生成不同工作流 | ✅ 已完成 |
| M3 后端落地 | 编排引擎 + Skill 调用 + 模型接入 + 评测迭代 | 🔲 可选 |
...
- **与 Coze 的边界**：如何讲清"垂直价值"而非"通用平台子集"——v0.4 的审片台/渠道集成/品牌中心构成差异化护城河。
...
*版本：v0.4.5。本文档随原型迭代更新。原型文件：`visionagent-prototype.html`（v0.4，11 页）+ 后端代理 `server.js`（本地真·Coze 调用）+ Railway 部署同一份 `server.js`（`visionagent-coze-proxy` 项目，`*.up.railway.app`，原 Cloudflare Worker `coze-worker.js` 因大陆访问不稳已弃用）；已部署公开 Demo：https://bf3ba5f1aca94473a9a402d7018112a5.sh4.agentos-app.net（进入「生产线」页输入自然语言后点「生成工作流」，即经 Railway 真实调用扣子智能体；若代理超时或网络异常则自动降级为演示数据，保证可看可交互）。真实能力验证：扣子智能体 [VisionAgent-生产线规划师](https://www.coze.cn/store/agent/7672806993245192246?bot_id=true)（NL→生产线 JSON，4 场景通过）。
