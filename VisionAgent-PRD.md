...
> 版本：v0.5.1（一期打通版 · 含 M3 落地设计 + 真实能力验证 + 闭环交互 + 图像/抠图/审核 API 接入 + 编排层健壮性 + 模型评测体系 + 数据飞轮）　|　作者：产品组　|　最后更新：2026-08-14　|　状态：原型阶段（前端 Demo，11 页可交互）＋ 真实可运行 Agent 已验证 ＋ 线上 Demo（GitHub Pages + 腾讯云 SCF）真·多模型调用打通 ＋ 真实抠图/出图/合规审核已接入 ＋ 闭环交互已实现 ＋ 万相抠图/调色真出图复测通过 ＋ 评测体系与反馈飞轮机制落地
...
### 11.9 模型评测体系（成本 · 延迟 · 质量三维）（v0.5.1 新增）
> 让"选模型"从拍脑袋变成有数据支撑的决策项，也是 AI PM 技术判断力的核心体现。

- **评测集设计**：按「行业 × 任务 × 难度」建 ≥20 条样本（电商 / 旅拍 / 餐饮 / 婚纱 / 3C × 拆线 / 分类 / 抠图 / 调色 / 出图 / 文案 × 明确 / 模糊 / 多图 / 跨平台）。每条结构：
  ```json
  { "id":"eval_001","industry":"电商","task":"拆线","difficulty":"明确",
    "input":"电商商品图做白底图发小红书",
    "expected_nodes":["素材入库","图片分类","智能抠图","场景生成","合规审核","多平台发布"],
    "expected_skills":["图片分类","智能抠图","场景生成","合规审核"],
    "pass_criteria":"节点覆盖率≥80%，含抠图+发布节点" }
  ```
- **三维指标**：
  - **成本**：各 mode 返回的 `cost` 真实累加；
  - **延迟**：curl `-w time_to_first_byte` + 前端计时 + 万相 / CogView 异步轮询次数；
  - **质量**：拆线节点覆盖率、出图主体一致性 / 审美（1–5）、分类 Top-1、文案平台适配度。
- **执行与产物**：Node 脚本读 `eval_cases.json` → 循环调 SCF → 写 `eval_results.json`；输出对比矩阵，直接指导默认模型路由策略（实测对比见 §11.10 表）。

### 11.10 数据飞轮与反馈回流闭环（v0.5.1 新增）
> AI 产品区别于普通功能产品的关键：上线即数据飞轮开始。把"审片反馈"从纯前端本地记忆升级为可回流、可复盘的迭代机制。

- **采集**：前端审片台「通过 / 打回」时 POST `mode:"feedback"`，携带 `runId / lineName / industry / node / source / rating(1–5) / action(pass|rework|regenerate) / feedback(批注) / workflow(JSON)`。
- **回流**：云函数 `mode:"feedback"` 仅 `console.log('[FEEDBACK] '+JSON.stringify(...))`，**零数据库、零新增配置**；SCF 日志 7 天可检索（前缀 `[FEEDBACK]`），后期可对接轻量 DB（如 COS + 定时导出）做结构化分析。
- **复盘机制（每周）**：

  | 复盘项 | 频率 | 操作 |
  |---|---|---|
  | 兜底率看板 | 每周 | 统计 `source` 含 `*_fallback` 的占比，定位易失败节点 |
  | 用户差评聚类 | 每周 | 按 `feedback` 关键词归类（白边 / 偏色 / 文案空泛等） |
  | Badcase 复盘 | 每周 | 挑 Top 3 失败类型 → 改 system prompt / 加灯塔模板 / 切默认模型 |
  | 模板沉淀 | 每月 | 把高赞 workflow 固化为灯塔模板 |

- **闭环链路**：用户运行 → 审片评分 → 日志回流 → 每周复盘 Top badcase → 改 prompt / 模板 / 模型 → 重新验证 → 体验提升 → 更多用户用。
- **价值**：把"我做了个 demo"升级为"我设计了能自迭代的产品机制"，是面试从"会调 API"进阶到"懂做 AI 产品"的关键信号。
...
| M2.7 模型评测体系 | 成本·延迟·质量三维评测集与对比矩阵，指导默认模型路由 | ✅（v0.5.1） |
| M2.8 数据飞轮 | 审片反馈 `[FEEDBACK]` 日志回流 + 每周复盘闭环，让 demo 自迭代 | ✅（v0.5.1） |
