...
- **数据复盘**：运行记录页顶部实时统计面板——运行成功率 / 打回重做率 / 平均耗时 / 累计消耗，反推优化（对应 PRD「评测体系」）。
- **模板与品牌联动**：「存为模板」保存**完整画布数据**（含 reworkLoop/品牌），「使用模板」一键恢复画布、无需重新调 Coze；品牌中心色板可一键应用到当前工作流「AI 调色」类节点，节点属性面板显示引用品牌。
...
- **成本**：智谱 CogView + 通义万相注册送免费额度，Demo 阶段成本≈0；商用按量计费。
...
| M2.5 真实能力验证 | 扣子可运行 Agent（NL→生产线 JSON，4 场景通过，[体验链接](https://www.coze.cn/store/agent/7672806993245192246?bot_id=true)）＋ 线上公开 Demo（[GitHub Pages](https://shalala-peng.github.io/visionagent-coze-proxy/) + 腾讯云 SCF 代理）真·多模型实时调用，大陆浏览器可按语义生成不同工作流 ＋ 闭环交互（运行/审片/打回/发布/复盘）＋ **真实视觉 Skill 接入**（通义万相图生图 / 真实抠图去背景 / 智谱出图兜底 / 合规审核规则引擎）＋ 编排层健壮性（Schema 校验 + 自动修正 + 带反馈重试） | ✅ 已完成 |
