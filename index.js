...
exports.main_handler = async (event, context) => {
  ...
  try {
    ...
    if (body.mode === 'matting') {
      ...
      try {
        let url, src = 'wanx_matting';
        try {
          url = await genMatting(imgs[0]);
        } catch (mErr) {
          // 万相失败（尺寸<512px/限流/地域）→ 兜底 CogView 出图，保证仍有可见结果（不保证去背景）
          url = await genImage('clean product photo on pure white background, studio lighting, no shadow');
          src = 'matting_fallback';
        }
        return buildResponse(200, { ok: true, url, mock: false, source: src, cost: src === 'wanx_matting' ? 0.06 : 0.1 });
      } catch (err) {
        const msg = String(err && err.message ? err.message : err);
    ...
    if (body.mode === 'color') {
      ...
      try {
        ...
        return buildResponse(200, { ok: true, url, mock: false, source: src, cost: src === 'wanx_color' ? 0.06 : 0.1 });
      } catch (err) {
        const msg = String(err && err.message ? err.message : err);
    ...
    if (!prompt || !String(prompt).trim()) {
      return buildResponse(400, { error: 'prompt 不能为空' });
    }
